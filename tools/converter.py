...
class Function(Object):
    ...
    def __init__(self, id, name):
        ...
        self.weight = None
        self.cycle = None
	self.filename = None #BWicht : In order to save the file name
...
class Profile(Object):
    ...
    def ratio(self, outevent, inevent):
        ...
        for function in self.functions.itervalues():
            assert outevent not in function
            assert inevent in function
            #BWicht : Replace ratio by the direct value : function[outevent] = ratio(function[inevent], self[inevent])
            function[outevent] = ratio(function[inevent], self[inevent])
            for call in function.calls.itervalues():
                assert outevent not in call
                if inevent in call:
                    #BWicht : Replace ratio by the direct value : call[outevent] = ratio(call[inevent], self[inevent])
...
class CallgrindParser(LineParser):
    ...
    def __init__(self, infile):
		...
		#BWicht : support for compressed file
        self.libraries = {'-1': 'error'}
    ...
    def parse_part(self):
        ...
        #for function in self.profile.functions.itervalues():
        #    print '\t', function.name, ' ==> ', function.module
    ...
    def parse_body_line(self):
        ...
        #BWicht : Improve module recuperation
        if line.startswith('ob='):
            ...
            #Compressed
            if lib.startswith('('):
                compressed = lib.split(' ')
                
                #Reference
                if len(compressed) == 1:
                    self.libraryob = self.libraries[compressed[0]]
                #New value
                else:
                    self.libraries[compressed[0]] = compressed[1]
                    self.libraryob = compressed[1]
            #Uncompressed
            else:
                self.libraryob = lib
        ...
        if line.startswith('cob='):
            ...
            if lib.startswith('('): 
                ...
                #Reference
                if len(compressed) == 1: 
                    self.librarycob = self.libraries[compressed[0]]
                #New value
                else: 
                    self.libraries[compressed[0]] = compressed[1]
                    self.librarycob = compressed[1]
            #Uncompressed
            else:  
                self.librarycob = lib
    ...
    def get_callee(self):
        ...
        return self.make_function(None, filename, function)
...
class DotWriter:
    ...
    def graph(self, profile, theme):
        ...
        #BWicht : Remove graphical informations
	    ...
	    filename = function.filename
            ...
            #BWicht : Remove graphical informations and add module
    ...
    def attr_list(self, attrs):
        ...
        for name, value in attrs.iteritems():
            #BWicht : Do not write empty property
            if value != "":
                if first:
                    first = False
                else:
